...
### `sh` — Shell
Shell de usuario que permita ejecutar las aplicaciones.
- Deberá contar con algún mecanismo para determinar si va a ceder o no el foreground al proceso que se ejecuta (por ejemplo, `&` al final de un comando para background).
- Deberá permitir conectar 2 procesos mediante un pipe con el símbolo `|`. No es necesario que permita conectar más de 2 procesos (es decir, `p1 | p2 | p3` no es requerido).
- Deberá proveer soporte para `Ctrl + D` (envío de end of file) y `Ctrl + C` (matar al proceso en foreground).

### `help`
Muestra una lista con todos los comandos disponibles. Deberá tener un apartado con el listado de tests provistos por la cátedra.
...
- `loop` — Imprime su ID con un saludo cada una determinada cantidad de tiempo. La espera deberá realizarse de forma activa (no bloquearse).
...
| `mvar 2 2` | Se mata al escritor B tras un tiempo | `ABABAAAAAA` |
...
El incumplimiento de cualquiera de ellos implica pasar a instancia de recuperatorio:

- Realizar la entrega hasta 96 horas después del deadline original.
- Incluir un README en la raíz del repositorio con la información detallada más adelante.
...
Es un requisito obligatorio para la compilación utilizar Makefile y la imagen provista por la cátedra:

```bash
docker pull agodio/itba-so-multiarch:3.1
```

Las reglas `make`, `make all`, `make <memory_manager>` deberán reservarse para tareas de compilación exclusivamente, a ser ejecutadas dentro de la imagen provista. Para cualquier otra tarea (descargar la imagen, iniciar el contenedor, instalar dependencias, ejecutar otras herramientas o iniciar qemu) se deberán usar reglas específicas.

---

## README

El repositorio deberá poseer un README en formato markdown con los siguientes ítems:
